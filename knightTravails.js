...
function knightTravails(startX, startY, endX, endY) {
    ...
    allNodes = getNodeslevelOrder(startNode); //Got a list with all nodes so far since the initial node
    unvisitedNodes = allNodes.filter(value => !visitedNodes.includes(value)); //Unvisited nodes contain all elements of allNodes except the ones that are also in visitedNodes
